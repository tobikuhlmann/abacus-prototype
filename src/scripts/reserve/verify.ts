...
import verification from '../../constants/reserve/remote_test_verification.json';
...
async function main() {
  ...
  const sourcePath = './contracts/reserve/MentoCrossChainReserveFlattened.sol';

  if (!existsSync(sourcePath)) {
    await execCmd(
      `npx hardhat flatten ./contracts/reserve/MentoCrossChainReserve.sol > ./contracts/reserve/MentoCrossChainReserveFlattened.sol`,
    );
...
// run with `yarn ts-node src/scripts/reserve/verify.ts`
