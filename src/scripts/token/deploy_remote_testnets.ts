...
async function main() {
  ...
  writeFileSync('./src/constants/token/remote_deployment_addresses.json', JSON.stringify(addresses));

  verificationJson.alfajores[0].name = "MentoPrototypeToken"
  verificationJson.alfajores[0].address = chainToContracts.alfajores.router.address
  ...
  writeFileSync('./src/constants/token/remote_test_verification.json', JSON.stringify(verificationJson));
}
