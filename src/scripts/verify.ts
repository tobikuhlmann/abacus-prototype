...
async function main() {
    ...
    //const core = new AbacusCore(abacus_core_mento_test, multiProvider); // if testnet2 environment is too much
    /*
    ...
    */
    ...
    if (!existsSync(sourcePath)) {
        await execCmd(`npx hardhat flatten ./contracts/HelloWorld.sol > ./contracts/HelloWorldFlattened.sol`);
        let flattenedSource = readFileSync(sourcePath, { encoding: 'utf8' });
        // Make SPDX license identifiers compiler compatible
        flattenedSource = flattenedSource.replace(/SPDX-License-Identifier:/gm, "License-Identifier:")
